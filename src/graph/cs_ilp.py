...
class CSILP: # Can-should ILP
  def __init__(
    ...
    self.forward_discount = ilp_args['forward_discount']
    self.backward_discount = ilp_args['backward_discount'] 
    self.positive_bias = ilp_args['positive_bias']
  ...
  def _data_by_option(self, comp, ops):
    ...
    for op_idx in range(self.num_option):
      ...
      for traj_comp, traj_ops in zip(comp, ops):
        ...
        # 2. weight
        # 2-1. weight for should=0
        option_times = should_labels.nonzero()[0].tolist()
        # compute should weight
        traj_len = should_labels.shape[0]
        t_since_option, t_until_option = np.zeros( (2, traj_len), dtype=int )
        for t in option_times:
            t_since_option[t:] = range(traj_len-t)
        for t in option_times[::-1]:
            t_until_option[:t+1] = range(t+1)[::-1]
    
        forward_weight = 1 - self.forward_discount ** t_until_option
        forward_weight[t_until_option==0] = 1.
        backward_weight = 1 - self.backward_discount ** t_since_option
        backward_weight[t_since_option==0] = 1.
        should_weight = np.minimum(forward_weight, backward_weight)
        
        # 2-1. weight for should=1
        should_weight[option_times] = self.positive_bias
